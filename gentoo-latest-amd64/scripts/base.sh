...
mount ${DEVICE}4 "$chroot" && cd "$chroot" && mkdir boot && mount ${DEVICE}1 boot
...
export DEVICE1_UUID=`blkid -o export ${DEVICE}1|grep UUID|grep -v PARTUUID`
export DEVICE3_UUID=`blkid -o export ${DEVICE}3|grep UUID|grep -v PARTUUID`
export DEVICE4_UUID=`blkid -o export ${DEVICE}4|grep UUID|grep -v PARTUUID`
